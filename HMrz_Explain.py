...
def load_nlg_dictionary():
    ...
    feature_dictionary = {'Goal Scored': ['scored no goal', 'scored {n} goal', 'scored {n} goals'],
        'Ball Possession %': ['no ball possession', 'possessed the ball {n}%', 'possessed the ball {n}%'],
        ...
        'Off-Target': ['had no shot off-target', 'had {n} shot off-target', 'had {n} shots off-target'],
        ...
        'Pass Accuracy %': ['had {n}% pass accuracy', 'had {n}% pass accuracy', 'had {n}% pass accuracy'],
        'Passes': ['had no pass', 'had {n} pass', 'had {n} passes'],
        'Distance Covered (Kms)': ['covered no distance', 'covered {n} kilometre distance', 'covered {n} kilometres distance'],
        ...
        'Yellow & Red': ['had no yellow and red card', 'had {n} yellow and red card', 'had {n} yellow and red cards'],
        'Red': ['had no red card', 'had {n} red card', 'had {n} red cards'],
        '1st Goal': ['scored no 1st goal', 'scored 1st goal at {n} minute', 'scored 1st goal at {n} minutes'],
        'Goals in PSO': ['had no goals in penalty shootouts', 'had {n} goal in penalty shootouts', 'had {n} goals in penalty shootouts'],
        'Own goals': ['suffered no own goal', 'suffered {n} own goal', 'suffered {n} own goals'],
        'Own goal Time': ['suffered no own goal in any minute', 'suffered 1st own goal at {n} minute', 'suffered 1st onw goal at {n} minutes'],}
...
def get_text_snippet(feature_dictionary, feature, value):
    ...
    using the language template dictionary
...
def explain_random_forest_shap(random_forest, shap_explainer, data_set, game_index):
    ...
    n_lines = 5  # number of supporting lines for the explanation
    ...
    while idx < shap_len:
        ...
        if support_count > n_lines and class_proba > 0.5:  # and idx > n_lines-1:
            print_debug_to_terminal(f"  - breaking, sup-lines = {support_count}, lines={idx}, class={class_proba}", l_out)
            break
...
def concatenate_feature_texts(feature_texts, indent = ' ', preamble=None):
    ...
    if 0 == t:
        ...
        feature_explanation = f"{feature_explanation}{indent}{feature_texts[0]} and<br>{indent}{feature_texts[1]}."
    else:
        ...
        for i, txt in enumerate(feature_texts):
            if 0 == i:
                ...
                feature_explanation = f"{feature_explanation}{indent}{txt}, and<br>"
            else:
...
def explain_counterfactual(random_forest, explanation_features, data_set, game_index):
    ...
    for feature in explanation_features:
        ...
        if obs_feature != cf_feature:
            counterfactuals.append(f"{cf_explanation} instead of {obs_feature}")
        # after the first research session it was clear that repeating all features rather than just the changed one
        # so for the following ones only the counterfactuals are displayed
        # else:
...
def generate_explanation(random_forest, shap_explainer, fifa_data, i):
    ...
    support_text = concatenate_feature_texts(supporting
                                             , '+ '
                                             , f'The model predicts this because, in order of importance, {team}')
    #                                        , f'The model predicts this with {proba}% probability because, in order of importance, {team}')

    # compile counter-factual text
...
def traverse_decision_tree(decision_tree, fifa_features, team, game_idx):
    ...
    node_name_dict = {0: 'Did the team fail to score a goal?',
                      ...
                      3: '1. Model prediction: the team did not have the MotM',
                      4: 'Did the team commit less than 9 fouls?',
                      5: '2. Model prediction: the team did not have the MotM',
                      6: '3. Model prediction: the team had the MotM',
                      7: '4. Model prediction: the team had the MotM',
                      8: 'Did the team have less than 3 shots off-target?',
                      9: 'Did the team have less than 77% accuracy?',
                      10: '5. Model prediction: the team had the MotM',
                      11: 'Did the team have less than 6 attempts?',
                      12: 'Did the team have less than 3 corners?',
                      13: '6. Model prediction: the team did not have the MotM',
                      14: '7. Model prediction: the team had the MotM',
                      15: '8. Model prediction: the team did not have the MotM',
                      ...
                      19: '9. Model prediction: the team did not have the MotM',
                      20: '10. Model prediction: the team had the MotM',
                      21: 'Did the team cover less than 148 km distance?',
                      22: '11. Model prediction: the team had the MotM',
                      23: '12. Model prediction: the team did not have the MotM',
                      24: '13. Model prediction: the team did not have the MotM', }
    ...
    for node_id in node_index:
        ...
        out_text = f"{node_text} {team} {snpt}, go to node <br>"
        path_description.append(out_text)
...
def main():
    ...
    # this particular implementation will return all generated explanation texts
    ...
    for i in range(128):
        ...
        team, game, label, proba = get_game_meta_data(fifa_data, i)
        #print(concatenate_feature_texts(supporting, '+ ',f'The model predicts this with {proba}% certainty because {team}'))
        print(concatenate_feature_texts(supporting, '+ ', f'The model predicts this because {team}'))
